...
export default function ExecutionPlanner() {
  ...
  const execute = () => {
    ...
    if (!result.ok) {
      executionLock.current = false;
      setMessage(result.blockers[0]?.message ?? 'Operacja została zatrzymana przez Reality Lock.');
      return;
    }
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-5 px-5 py-6">
      <section className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5">
        <div className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.2em] text-emerald-300"><ShieldCheck className="h-4 w-4" /> Reality Lock · jedna ścieżka wykonania</div>
        <p className="mt-2 text-sm text-white/55">Plan można oglądać jako symulację. Fizyczne wykonanie wymaga VERIFIED + PHYSICAL_ALLOWED, wszystkich potwierdzonych kroków, prawidłowych pomiarów, zapasu preparatów i mierzalnego zestawu narzędzi.</p>
      </section>

      <section className="grid gap-3 md:grid-cols-3 xl:grid-cols-6">
        <Select label="Medium" value={store.currentMedium} onChange={value => store.updateMedium(value as typeof store.currentMedium)} options={['TERRA', 'COCO', 'HYDRO', 'CUSTOM']} />
        ...
        <div className="rounded-xl border border-white/10 bg-white/5 p-3">
          <label className="text-[10px] font-bold uppercase tracking-wider text-white/35">{isRts ? 'Zużycie RTS ml' : 'Woda bazowa L'}</label>
      ...
      <section className="grid gap-5 lg:grid-cols-12">
        <div className="space-y-4 lg:col-span-4">
            ...
            <div className="space-y-2">
              {recipes.length ? recipes.map(item => (
                <button key={item.id} type="button" onClick={() => setRecipeId(item.id)} className={`w-full rounded-xl border p-3 text-left ${recipeId === item.id ? 'border-emerald-500/40 bg-emerald-500/10' : 'border-white/10 bg-black/30'}`}>
                  <div className="flex items-center justify-between gap-2"><span className="text-sm font-bold">{item.name}</span><StatusBadge text={item.verificationStatus ?? 'UNVERIFIED'} ok={item.verificationStatus === 'VERIFIED'} /></div>
                  <div className="mt-1 text-[10px] text-white/35">{item.executionPolicy ?? 'SIMULATION_ONLY'} · W{item.weekStart ?? '*'}–{item.weekEnd ?? '*'}</div>
                </button>
          ...
          {recipe && (
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              ...
              <div className="mt-3 space-y-2">
                {simulationWarnings.length ? simulationWarnings.map((warning, index) => <Notice key={`${warning.code}-${index}`} text={warning.message} error={warning.severity === 'ERROR'} />) : <Notice text="Kontekst receptury jest spójny." />}
              </div>
            </div>
          )}
          ...
          {readiness && (
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <div className="flex items-center justify-between gap-2"><span className="text-xs font-black uppercase tracking-wider text-white/40">Physical gate</span><StatusBadge text={readiness.allowed ? 'READY' : 'LOCKED'} ok={readiness.allowed} /></div>
              <div className="mt-3 max-h-72 space-y-2 overflow-auto">
                {readiness.blockers.length ? readiness.blockers.map((blocker, index) => <Notice key={`${blocker.code}-${index}`} text={blocker.message} error />) : <Notice text="Wszystkie twarde bramki zaliczone." />}
              </div>
            </div>
          )}
        </div>
                    ...
                    return (
                      <div key={step.id} className={`rounded-xl border p-4 ${checked ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-cyan-500/20 bg-cyan-500/5'}`}>
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1"><div className="text-[10px] uppercase tracking-widest text-cyan-300/70">Krok {index + 1} · gate</div><div className="font-bold">{step.title}</div><div className="mt-1 text-xs text-white/45">{step.detail}</div>{step.measurement && <MeasurementInput type={step.measurement} measurements={measurements} onChange={setMeasurement} />}</div>
                        </div>
                      </div>
                    );
                  }
