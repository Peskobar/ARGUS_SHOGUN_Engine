...
export default function RecipeBuilderPanel() {
  ...
  const save = () => {
    ...
    setMessage(ok ? 'Zapisano jako UNVERIFIED / SIMULATION_ONLY. Osobny audyt może później nadać autoryzację.' : 'Nie udało się zapisać receptury.');
    if (ok) { setName(''); setIngredients([]); }
  };

  return (
    <main className="mx-auto max-w-5xl px-5 py-6">
      <div className="mb-5 flex items-center gap-2"><Settings className="h-5 w-5 text-amber-300" /><div><h2 className="font-black">Kreator receptur</h2><p className="text-xs text-white/40">Własna receptura zawsze startuje jako UNVERIFIED / SIMULATION_ONLY. Kreator nie może sam nadać sobie uprawnień wykonawczych.</p></div></div>
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <section className="rounded-2xl border border-white/10 bg-white/5 p-5"><div className="mb-3 text-xs font-bold uppercase tracking-wider text-white/35">Skład i jawny mixOrder</div><div className="space-y-2">{ingredients.map((ingredient, index) => { const product = store.getProduct(ingredient.productId); return product ? <div key={ingredient.productId} className="flex items-center gap-2 rounded-xl border border-white/10 bg-black/30 p-3"><span className="w-6 font-mono text-xs text-white/30">{index + 1}</span><span className="flex-1 text-sm font-bold">{product.name}</span>{method !== ApplicationMethod.READY_TO_SPRAY && <input className="w-20 rounded-lg border border-white/10 bg-black px-2 py-1 text-right font-mono text-sm" type="number" min="0" step="0.1" value={ingredient.concentration} onChange={event => setIngredients(current => current.map(item => item.productId === ingredient.productId ? { ...item, concentration: Math.max(0, Number(event.target.value) || 0) } : item))} />}<span className="text-[10px] text-white/35">{method === ApplicationMethod.READY_TO_SPRAY ? 'RTS' : 'ml/L'}</span><button type="button" onClick={() => setIngredients(current => normalize(current.filter(item => item.productId !== ingredient.productId)))} className="text-red-300">×</button></div> : null; })}{ingredients.length === 0 && <div className="rounded-xl border border-dashed border-white/10 p-6 text-center text-sm text-white/30">Brak składników</div>}</div><button type="button" onClick={save} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-amber-400 px-4 py-3 font-black text-black"><Save className="h-4 w-4" />Zapisz do symulacji</button>{message && <div className="mt-3 rounded-xl border border-white/10 bg-black/30 p-3 text-sm text-white/60">{message}</div>}</section>
      </div>
    </main>
  );
}
